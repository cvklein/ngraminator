...
```--print_separators``` Show the default list of separators.   
...
The code to find the largest non-overlapping match has a bit of a brute force component. This doesn't matter that much because Chinese texts are pretty small and modern computers are fast. However, if you put in a single big file (so, from experience, the entire *Shiji*) it can take upwards of a few minutes to run. It will look hung while it runs. It's a good time for a cup of tea. Or you can split your file up into smaller chapters; in general, smaller files are much faster.
